...
- 신규: `backend/app/services/report_service.py` — Jinja2 HTML 템플릿 → PDF 변환
- 신규: `backend/app/routers/reports.py` — `GET /reports/health-pdf/{pet_id}?date_from=&date_to=`
- 신규: `backend/app/templates/health_report.html` — PDF 템플릿
- `backend/requirements.txt`에 `weasyprint` 추가
- `backend/Dockerfile`에 `libcairo2`, `libpango` 시스템 패키지 추가
- Premium 전용 (tier 체크)

**Flutter:**

- `pubspec.yaml`에 `share_plus` 추가
- 건강체크 히스토리 / BHI 화면에 PDF 다운로드/공유 버튼 (premium only)

### 3-2. 병원 방문용 요약 리포트

**백엔드:**

- `report_service.py` 확장 — `generate_vet_summary()` (최근 체중 추이, 건강체크 5건, BHI 이력, 이상 소견, 급여 패턴)
- `GET /reports/vet-summary/{pet_id}` 엔드포인트

**Flutter:**

- 신규: `lib/src/screens/reports/vet_summary_screen.dart`
- 펫 프로필 또는 건강체크 히스토리에서 진입
- Premium 전용 + PDF 공유
...
| 신규 | `backend/app/routers/reports.py`                               |
| 신규 | `backend/app/templates/health_report.html`                     |
| 수정 | `backend/app/scheduler.py`                                     |
| 수정 | `backend/app/main.py` (reports 라우터 등록)                    |
| 수정 | `backend/requirements.txt` (`weasyprint`)                      |
| 수정 | `backend/Dockerfile` (시스템 패키지)                           |
...
- [ ] PDF 리포트 생성 및 다운로드/공유
- [ ] 병원용 요약 리포트 정확한 데이터 포함
- [ ] 건강 변화 카드 홈 화면 표시
- [ ] 주간 인사이트 스케줄러 정상 동작
