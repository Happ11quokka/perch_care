...
**작성일:** 2026-03-07
**문서 상태:** v2 (Phase 3-3, 3-4 구현 완료 반영, 2026-03-08)
...
- Free: 기본 요약 (BHI, 체중, 추세) / Premium: 상세 카드 (이상 소견 30일, 급여·음수 일관성, BHI 추세)
- `backend/app/services/health_summary_service.py` — 집계 서비스
- `backend/app/schemas/health_summary.py` — Pydantic 응답 스키마

**Flutter:**

- 신규: `lib/src/widgets/health_summary_card.dart` — Free: blur+잠금 오버레이+paywall CTA, Premium: 상세 카드
- 신규: `lib/src/models/health_summary.dart` — 프론트엔드 모델
- 홈 화면 WCI 카드 아래, AI 배너 아래에 표시
- Premium 구매 후 복귀 시 자동 갱신 (`.then()` 콜백)

**적용된 버그 수정:**

- **(P1)** `_count_abnormals`: `status == "abnormal"` → `status != "normal"` (실제 값은 `normal|caution|warning|critical`)
- **(P1)** `_calc_consistency`: `(until - since).days` → `.days + 1` (inclusive 범위 off-by-one 수정, 100% 초과 방지)

### 3-4. 주간 인사이트 ✅

> **변경 사항:** 계획에서 APScheduler 사용 → Railway Cron Job 전용으로 변경. `daily_reminder` 패턴과 동일하게 `python -m app.jobs.weekly_insights`로 실행. APScheduler에서 weekly_insights_job을 제거하여 중복 실행 방지.

> **변경 사항:** 계획에서 주간/월간 → 주간만 구현. `monthly` 옵션은 생성 파이프라인이 없어 API 패턴에서 제거 (`^(weekly)$`).

**백엔드:**

- `backend/app/jobs/weekly_insights.py` — Railway Cron Job 엔트리포인트 (`0 0 * * 1`, UTC 00:00 Monday = KST 09:00)
- `backend/app/services/insights_service.py` — GPT-4o-mini 기반 주간 데이터 집계 + 자연어 인사이트 생성
- `backend/app/models/pet_insight.py` — `pet_insights` 테이블 모델 (composite index: `pet_id, insight_type, period_end`)
- `backend/app/schemas/pet_insight.py` — Pydantic 응답 스키마
- `backend/alembic/versions/016_add_pet_insights_table.py` — 마이그레이션
- `GET /pets/{pet_id}/insights?type=weekly` — Premium 전용 (403 for free)
- `backend/app/scheduler.py` — weekly_insights_job 제거 (Railway Cron만 사용)
- `backend/app/routers/pets.py` — `type` 패턴 `^(weekly)$`로 제한

**Flutter:**

- 신규: `lib/src/models/pet_insight.dart` — 프론트엔드 모델
- 홈 화면 인사이트 섹션: Premium → 요약 + 추천사항 카드, Free → 티저 카드 + paywall CTA
- FCM 푸시 알림: 주간 인사이트 생성 완료 시 다국어 (zh/ko/en) 알림 발송
- Premium 구매 후 복귀 시 인사이트 자동 로드 (`.then()` 콜백)

**적용된 버그 수정:**

- **(P1)** `_aggregate_weekly_data` 건강체크 집계: `status == "abnormal"` → `status != "normal"`
- **(P2)** 마지막 날 건강체크 누락: `checked_at <= period_end` (date) → `checked_at < period_end + timedelta(days=1)`
- **(P2)** Premium 구매 후 홈 미갱신: `pushNamed` → `.then(() => _loadHealthSummaryAndInsights())`
- **(P3)** monthly dead-end 제거: API 패턴에서 `monthly` 제거

**운영 참고:**

- Railway Cron 설정 필요: `0 0 * * 1` / `python -m app.jobs.weekly_insights`
- 기본 언어: `zh` (90%+ 중국어 사용자 기반, `daily_reminder.py`와 동일)
- GPT 실패 시 빈 레코드 저장 → 해당 주에 재생성 불가 (dedup 로직). 향후 개선 가능
...
### Phase 3 (3-3, 3-4) 파일 총 목록

| 작업 | 파일                                                                  | 상태 |
| ---- | --------------------------------------------------------------------- | ---- |
| 신규 | `backend/app/services/health_summary_service.py`                      | ✅   |
| 신규 | `backend/app/services/insights_service.py`                            | ✅   |
| 신규 | `backend/app/jobs/weekly_insights.py` (Railway Cron 엔트리포인트)     | ✅   |
| 신규 | `backend/app/models/pet_insight.py`                                   | ✅   |
| 신규 | `backend/app/schemas/health_summary.py`                               | ✅   |
| 신규 | `backend/app/schemas/pet_insight.py`                                  | ✅   |
| 신규 | `backend/alembic/versions/016_add_pet_insights_table.py`              | ✅   |
| 수정 | `backend/app/routers/pets.py` (health-summary + insights 엔드포인트)  | ✅   |
| 수정 | `backend/app/models/__init__.py` (PetInsight 등록)                    | ✅   |
| 수정 | `backend/app/scheduler.py` (weekly_insights_job 제거, Railway Cron 전용) | ✅   |
| 신규 | `lib/src/widgets/health_summary_card.dart`                            | ✅   |
| 신규 | `lib/src/models/health_summary.dart`                                  | ✅   |
| 신규 | `lib/src/models/pet_insight.dart`                                     | ✅   |
| 수정 | `lib/src/screens/home/home_screen.dart` (카드 + 인사이트 + 갱신 콜백) | ✅   |
| 수정 | `lib/l10n/app_ko.arb`, `app_en.arb`, `app_zh.arb` (10개 키 추가)     | ✅   |
...
- [ ] `GET /pets/{petId}/health-summary` Free tier → 기본 필드만 반환 (abnormal_count 등 null)
- [ ] `GET /pets/{petId}/health-summary` Premium tier → 상세 필드 포함 (abnormal_count, food/water consistency, bhi_trend)
- [ ] `status="caution"` 건강체크 생성 후 → `abnormal_count > 0` 확인 (P1 수정)
- [ ] 30일 매일 기록 시 → `food_consistency`, `water_consistency` ≤ 100.0 (P1 수정)
- [ ] 건강 변화 카드 홈 화면 표시 (WCI 카드 아래)
- [ ] Free 사용자 → blur 오버레이 + "Premium 업그레이드" CTA
- [ ] `GET /pets/{petId}/insights?type=weekly` Free → 403
- [ ] `GET /pets/{petId}/insights?type=weekly` Premium → 인사이트 반환 (또는 null)
- [ ] `GET /pets/{petId}/insights?type=monthly` → 422 validation error (P3 수정)
- [ ] 일요일 오후 건강체크 → 주간 인사이트 `health_check_total`에 포함 (P2 수정)
- [ ] Railway Cron `python -m app.jobs.weekly_insights` → 월요일 인사이트 1회 생성
- [ ] APScheduler에서 weekly_insights_job 미등록 확인 (중복 실행 방지)
- [ ] Premium 구매 CTA → 구매 완료 → 홈 복귀 시 카드/인사이트 즉시 갱신 (P2 수정)
- [ ] FCM 푸시 알림 수신 (zh/ko/en 다국어)
