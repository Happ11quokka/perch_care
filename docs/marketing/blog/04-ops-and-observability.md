...
여기에 **KB 공백 자동 감지**를 얹는다. 검색 결과의 평균 유사도(*커버리지* — 검색 결과가 질문을 얼마나 잘 덮는지 보여주는 지표)가 **0.3 미만**이면 다음 로그가 남는다:
...
1~3편의 결정 5개와 4편의 그레이스풀 디그라데이션을 한 표 위에 놓는다.
...
한 사용자의 가장 좋은 답변보다, 모든 사용자에게 같은 신뢰선. 그것이 우리가 4편에 걸쳐 양보한 것들과 지킨 것들이 합쳐지는 지점이다.
