...
class _EmailLoginScreenState extends ConsumerState<EmailLoginScreen> {
  ...
  /// 인증 성공 후 라우팅. [hasPets] tri-state → 목적지 결정.
  /// 재진입 가드로 중복 네비게이션을 막는다.
  void _navigateAfterLogin(bool? hasPets) {
    if (!mounted || _hasNavigatedAfterLogin) return;
    _hasNavigatedAfterLogin = true;

    switch (destinationForHasPets(hasPets)) {
      case PostLoginDestination.onboarding:
  ...
  /// 소셜 로그인 결과 처리
  void _handleSocialLoginResult(LoginOutcome outcome) {
    switch (outcome) {
      case LoginAuthenticated(:final hasPets):
        _navigateAfterLogin(hasPets);
      case LoginSignupRequired():
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      final outcome = await ref.read(authViewModelProvider.notifier).signInWithEmail(
            email: email,
            password: password,
          );
      if (!mounted) return;
      // 이메일 로그인은 항상 LoginAuthenticated를 반환한다.
      if (outcome is LoginAuthenticated) {
        _navigateAfterLogin(outcome.hasPets);
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _handleGoogleLogin() async {
    ...
    final l10n = AppLocalizations.of(context);
    try {
      final outcome = await ref.read(authViewModelProvider.notifier).loginWithGoogle();
      if (!mounted) return;
      if (outcome == null) return; // 사용자 취소 — 에러 없이 조용히 종료
      _handleSocialLoginResult(outcome);
    } catch (e, stackTrace) {
  ...
  Future<void> _handleAppleLogin() async {
    ...
    final l10n = AppLocalizations.of(context);
    try {
      final outcome = await ref.read(authViewModelProvider.notifier).loginWithApple();
      if (!mounted) return;
      if (outcome == null) return; // 사용자 취소 — 에러 없이 조용히 종료
      _handleSocialLoginResult(outcome);
    } catch (e) {
