...
IP는 sha256(ip + demo_api_key) 단방향 해시로만 저장하여 원본을 보존하지 않는다.
...
def hash_ip(ip: str) -> str:
    """sha256(ip + demo_api_key). 원본 IP를 저장하지 않기 위한 단방향 해시."""
    return hashlib.sha256((ip + get_settings().demo_api_key).encode()).hexdigest()
...
async def check_and_reserve(db: AsyncSession, ip: str, kind: str) -> dict:
    ...
    # 슬롯 예약
    db.add(DemoUsageLog(ip_hash=ip_hashed, kind=kind))
    await db.flush()
